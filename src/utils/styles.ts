...
export const toPropValue = <T>(
  ...
  if (isResponsiveType(prop)) {
    const result = [];
    for (const responseKey in prop) {
      if (responseKey === "base") {
        result.push(
        ...
        // メディアクエリでのスタイル
        const breakpoint = BREAKPOINTS[responseKey];
...
function isResponsiveType<T>(prop: any): prop is ResponsiveProp<T> {
  return (
    prop &&
    (prop.base !== undefined ||
      prop.sm !== undefined ||
